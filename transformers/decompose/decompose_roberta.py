...
import itertools
from decompose_util import decomp_activation, LinearDecomposed
from decompose_util import LayerNormDecomposed as RobertaLayerNormDecomposed
...
class RobertaSelfAttentionDecomposed:
    def __init__(self, layer: RobertaSelfAttention, debug=False):
...
class RobertaSelfOutputDecomposed:
    def __init__(self, layer: RobertaSelfOutput, debug=False):
        self.layer = layer.to(torch.torch.double)
        self.dense_decomposed = LinearDecomposed(self.layer.dense, debug=debug)
        self.LayerNorm_decomposed = RobertaLayerNormDecomposed(
...
class RobertaAttentionDecomposed:
    def __init__(self, layer: RobertaAttention, debug=False):
        self.layer = layer.to(torch.torch.double)
        self.self_decomposed = RobertaSelfAttentionDecomposed(
            self.layer.self, debug)
        self.output_decomposed = RobertaSelfOutputDecomposed(
...
class RobertaIntermediateDecomposed:
    def __init__(self, layer: RobertaIntermediate, debug=False):
        self.layer = layer.to(torch.torch.double)
        self.dense_decomposed = LinearDecomposed(self.layer.dense, debug=debug)
        self.debug = debug
...
class RobertaOutputDecomposed:
    def __init__(self, layer: RobertaOutput, debug=False):
        self.layer = layer.to(torch.torch.double)
        self.dense_decomposed = LinearDecomposed(self.layer.dense, debug=debug)
        self.LayerNorm_decomposed = RobertaLayerNormDecomposed(
            self.layer.LayerNorm, debug=debug)
...
class RobertaLayerDecomposed:
    def __init__(self, layer: RobertaLayer, debug=False):
        self.layer = layer.to(torch.torch.double)
        self.attention_decomposed = RobertaAttentionDecomposed(
            self.layer.attention, debug=debug)
        self.intermediate_decomposed = RobertaIntermediateDecomposed(
            self.layer.intermediate, debug=debug)
        self.output_decomposed = RobertaOutputDecomposed(
...
class RobertaEncoderDecomposed:
    def __init__(self, model: RobertaEncoder, debug=False):
        self.model = model.to(torch.torch.double)
        self.layer_decomposed = [RobertaLayerDecomposed(
            layer, debug=debug) for layer in self.model.layer]
        self.debug = debug
    ...
    def __call__(self, xs, attention_mask, perms):
        ...
        for i, layer_module in enumerate(self.layer_decomposed):
            ...
            if self.debug:
                ...
                print(f"Roberta Encoder Layer {i} error: {error}")

        return xs


class RobertaPoolerDecomposed:
    def __init__(self, layer: RobertaPooler, debug=False):
        self.layer = layer.to(torch.torch.double)
        self.dense_decomposed = LinearDecomposed(self.layer.dense, debug=debug)
        self.debug = debug
...
class RobertaEmbeddingsDecomposed:
    def __init__(self, layer: RobertaEmbeddings, debug=False):
        self.layer = layer.to(torch.torch.double)
        self.LayerNorm_decomposed = RobertaLayerNormDecomposed(
            self.layer.LayerNorm, debug=debug)
        self.debug = debug

    def __call__(self, input_ids, beta_mask, num_contributions, token_type_ids=None,
                 past_key_values_length=0):
        if input_ids is not None:
            position_ids = create_position_ids_from_input_ids(
                input_ids, self.layer.padding_idx, past_key_values_length)
        ...
        if self.debug:
            ...
            embeddings_ = self.layer(
                input_ids, token_type_ids=None, past_key_values_length=0)
            assert torch.allclose(
...
class RobertaModelDecomposed:
    def __init__(self, model: RobertaModel, debug=False):
        self.model = model.to(torch.torch.double)
        self.embeddings_decomposed = RobertaEmbeddingsDecomposed(
            self.model.embeddings, debug=debug)
        self.encoder_decomposed = RobertaEncoderDecomposed(
            self.model.encoder, debug=debug)
        self.pooler_decomposed = RobertaPoolerDecomposed(
...
class RobertaForSequenceClassificationDecomposed:
    def __init__(self, config=None, model=None, num_labels=2, debug=False, shapley_include_bias=False, num_contributions=2):
        ...
        if config is None and model is None:
            raise ValueError("Need to provide either Roberta config or model")
        self.bert_decomposed = RobertaModelDecomposed(
            self.model.bert, debug=debug)
    ...
    def __call__(self, input_ids, beta_mask, token_type_ids=None,
        ...
        if self.debug:
            ...
            print("\nRoberta Classifier logits error: ", logits_error)
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model, num_labels=2, num_contributions=2, shapley_include_bias=False, debug=False):
        """
        load from pretrained model name or file path from huggingface
        """
        model_hf = transformers.RobertaForSequenceClassification.from_pretrained(
            pretrained_model,
            num_labels=num_labels,
        )
        ...
        decomposed_model = RobertaForSequenceClassificationDecomposed(
            config=RobertaConfig.from_dict(model_hf.config.to_dict()),
            num_labels=num_labels,
            debug=debug,
            num_contributions=num_contributions,
...
class RobertaPredictionHeadTransformDecomposed:
    def __init__(self, layer: RobertaPredictionHeadTransform, debug=False):
        self.layer = layer.to(torch.torch.double)
...
class RobertaLMPredictionHeadDecomposed:
    def __init__(self, layer: RobertaLMPredictionHead, debug=False):
        self.layer = layer.to(torch.torch.double)
        self.transform_decomposed = RobertaPredictionHeadTransformDecomposed(
...
class RobertaOnlyMLMHeadDecomposed:
    def __init__(self, layer: RobertaOnlyMLMHead, debug=False):
        self.layer = layer.to(torch.torch.double)
        self.predictions_decomposed = RobertaLMPredictionHeadDecomposed(
...
class RobertaForMaskedLMDecomposed:
    def __init__(self, config, debug=False, shapley_include_bias=False, num_contributions=2):
        self.model = RobertaForMaskedLM(
            config).to(torch.torch.double)
        self.bert_decomposed = RobertaModelDecomposed(
            self.model.bert)
        self.cls_decomposed = RobertaOnlyMLMHeadDecomposed(
            self.model.cls)
        self.debug = debug
    ...
    def __call__(self, input_ids, beta_mask, token_type_ids=None, attention_mask=None):
        ...
        if self.debug:
            ...
            print("\nRoberta LM predictions error: ", predictions_error)

        return prediction_scores

    @classmethod
    def from_pretrained(cls, pretrained_model, debug=False, num_contributions=2, shapley_include_bias=False, **kwargs):
        """
        load from pretrained model name or file path from huggingface
        """
        model_hf = transformers.RobertaForMaskedLM.from_pretrained(
        ...
        decomposed_model = RobertaForMaskedLMDecomposed(
            config=RobertaConfig.from_dict(model_hf.config.to_dict()),
            debug=debug,
            num_contributions=num_contributions,
            shapley_include_bias=shapley_include_bias
